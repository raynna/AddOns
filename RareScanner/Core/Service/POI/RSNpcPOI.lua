...
local function IsNpcPOIFiltered(npcID, mapID, artID, zoneQuestID, prof, group, questTitles, onWorldMap, onMinimap)
	...
	if (not RSConfigDB.IsShowingOtherRareNPCs() and not isAchievement and not prof) then
...
function RSNpcPOI.GetMapNotDiscoveredNpcPOIs(mapID, questTitles, onWorldMap, onMinimap)
	...
	for npcID, _ in pairs(notDiscoveredNpcIDs) do
		...
		if (not filtered and not IsNpcPOIFiltered(npcID, mapID, RSNpcDB.GetInternalNpcArtID(npcID, mapID), npcInfo.zoneQuestId, npcInfo.prof, npcInfo.group, questTitles, onWorldMap, onMinimap)) then
...
function RSNpcPOI.GetMapAlreadyFoundNpcPOI(npcID, alreadyFoundInfo, mapID, questTitles, onWorldMap, onMinimap)
	...
	if (not IsNpcPOIFiltered(npcID, mapID, alreadyFoundInfo.artID, zoneQuestID, prof, group, questTitles, onWorldMap, onMinimap)) then
